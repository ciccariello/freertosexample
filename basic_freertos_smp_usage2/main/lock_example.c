...
static void inc_num_mutex(void *arg) {
	...
	while (!timed_out) {
		...
		int core_id = esp_cpu_get_core_id();
		ESP_LOGI(TAG, "task%d read value = %d on core #%d", task_index,
				s_global_num, core_id);
		s_global_num++;
...
//		xTaskCreatePinnedToCore(inc_num_mutex_iter, NULL, 4096, NULL,
//				TASK_PRIO_3, NULL, core_id);
//	}
//
//	// reset s_global_num
...
//	for (core_id = 0; core_id < CONFIG_FREERTOS_NUMBER_OF_CORES; core_id++) {
//		xTaskCreatePinnedToCore(inc_num_spinlock_iter, NULL, 4096, NULL,
//				TASK_PRIO_3, NULL, core_id);
//	}
//
//	vTaskDelay(2000 / portTICK_PERIOD_MS);
...
//	for (core_id = 0; core_id < CONFIG_FREERTOS_NUMBER_OF_CORES; core_id++) {
//		xTaskCreatePinnedToCore(inc_num_atomic_iter, NULL, 4096, NULL,
//				TASK_PRIO_3, NULL, core_id);
//	}
	...
	for (thread_id = 0; thread_id < SHARE_RES_THREAD_NUM; thread_id++) {
		xTaskCreatePinnedToCore(inc_num_mutex, NULL, 4096, (void*) thread_id,
				TASK_PRIO_3, NULL, tskNO_AFFINITY);
	}
